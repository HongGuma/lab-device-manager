...
const LabContent = ({entryID,entryName}) => {
    ...
    const crrentURL = 'http://210.218.217.110:3103/api/getLabData.php'; //데이터 출력시 api url
    ...
    useEffect(()=>{
        const fetchList = async () => {
            try{
                ...
                setList(null);
                setLoading(null);
                const res = await axios.get(crrentURL+'?parm=equipment&entry_id='+currentId); //equipment 불러오는 axios
                const res2 = await axios.get(crrentURL+'?parm=count&entry_id='+currentId); //count 불러오는 axios
                setList(res.data);
                setNum(res2.data);
            }catch (e){
                setError(e);
    ...
    async function openDelete(){
        if(checkedItems.size > 0){
            for(let id of checkedItems){    //checkedItems에 있는 id를 하나씩 꺼낸다.
                await axios.get(deleteURL + '?table=lab&item_id=' + id)
                    .then((res) => {
                        // console.log(res)
                    });
            }
