...
import {
  Select,
  filteringConsentPosts,
  sortConsentPosts,
  exportFile,
} from "./TabContainerFunc";
...
const TabContainer = ({
  ...
  const [currentPosts, setCurrentPosts] = useState(
    currentList.slice(indexOfFirst, indexOfLast)
  ); //현재 리스트 자른거
    ...
    if (window.confirm(checkedItems.size + "개의 데이터를 삭제하시겠습니까?")) {
      if (checkedItems.size > 0) {
        for (let item of checkedItems) {
          await axios
            .post(URL, { parm: "delete", table: table, id: item })
            .then((res) => {
  ...
  function onClickSearchBtn() {
    ...
    const filtering = filteringConsentPosts(
      inputSearchText,
      posts,
      selectSearchValue
    );
      ...
      <div className="tab-table">
        {tabNum === 1 && (
          <ConsentContents
            ...
            onClickConsentInsertDone={onClickConsentInsertDone}
        ...
        {tabNum === 3 && (
          <MedicalCheckupContents
            URL={URL}
            medicalPosts={currentPosts}
            currentEntry={currentEntry}
            onCheckSingle={onCheckSingle}
            checkedItems={checkedItems}
            onCheckAll={onCheckAll}
