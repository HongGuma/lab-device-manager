            ...
            <ul className="insert-ul">
                {
                    medicalEntry.map((item)=>(
                        <li key={item.id}><input type="textbox" name={"r_"+item.id} onChange={(e)=>insertHandler(e)}/></li>
                    ))
                }
            </ul>
    ...
    function onKeyPress(e){
        if(e.key === 'Enter'){
            if(window.confirm('수정 하시겠습니까?')){
                axios.post(URL,{parm:'medicalUpdate',col_nm:dubleClickedItem,update_data:updateItem,id:dubleClickedID})
                    .then((res)=>{
                        if(res.data){
                            alert("수정 완료.");
                        }else{
