...
const Pagination = ({
  ...
  } //하단 넘버링 화살표 누르면 다음 숫자 리스트 출력하는 함수
  function onClickMovePre() {
    setCurrentNumPage(currentNumPage - 1);
  } //하단 넘버링 화살표 누르면 이전 숫자 리스트 출력하는 함수
  function onClickMoveEnd() {
    //마지막 페이지로 이동
    setCurrentNumPage(endPage);
    paginate(endNum);
    setClickNum(endNum);
  }
  function onClickMoveBeginning() {
    //1번 숫자로 돌아감
    setCurrentNumPage(1);
    paginate(1);
    setClickNum(1);
  ...
  return (
    <ul className="pagination">
      {currentNumPage > 1 && (
        <li onClick={onClickMoveBeginning}>
          <p> ◁ </p>
        </li>
      )}
      {currentNumPage > 1 && (
        <li onClick={onClickMovePre}>
          {" "}
          <p> ◀ </p>{" "}
        </li>
      )}
      {currentNum.map((pageNum) => (
        <li
          className={clickNum === pageNum ? "active" : ""}
          key={pageNum}
          onClick={() => onClickPaginate(pageNum)}
        >
          <p>{pageNum}</p>
        </li>
      ))}
      {currentNumPage < numberPages / perNum && (
        <li onClick={onClickMoveNext}>
          {" "}
          <p> ▶ </p>{" "}
        </li>
      )}
      {currentNumPage < numberPages / perNum && (
        <li onClick={onClickMoveEnd}>
          <p> ▷ </p>
        </li>
      )}
    </ul>
