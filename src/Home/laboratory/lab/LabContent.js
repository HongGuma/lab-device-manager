  ...
  return (
    <form onSubmit={clickEvent}>
      <ul className="body-ul">
        <li></li>
        <li>
          <input name="asset_num" type="textbox" onChange={changeHandler} />
        </li>
        <li>
          <input name="name" type="textbox" onChange={changeHandler} />
        </li>
        <li>
        ...
        <li>
          <input name="position" type="textbox" onChange={changeHandler} />
        </li>
        <li>
          <input
            name="issue_date"
...
const LabContent = ({ entryID, entryName }) => {
  ...
  const crrentURL = "http://localhost:3103/api/getLabData.php"; //데이터 출력시 api url
    ...
    const fetchList = async () => {
      try {
        ...
        const res = await axios.get(
          crrentURL + "?parm=equipment&entry_id=" + currentId
        ); //equipment 불러오는 axios
        const res2 = await axios.get(
          crrentURL + "?parm=count&entry_id=" + currentId
        ); //count 불러오는 axios
        setList(res.data);
        setNum(res2.data);
      } catch (e) {
  ...
  async function openDelete() {
    if (checkedItems.size > 0) {
      for (let id of checkedItems) {
        ...
        await axios.get(deleteURL + "?table=lab&item_id=" + id).then((res) => {
          // console.log(res)
        });
      }
      setDelete(true);
