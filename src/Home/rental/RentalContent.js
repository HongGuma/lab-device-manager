 ...
 */
const AllItem = ({list, oneCheckHandler, allCheckHandler, checkedAll, checkedList}) => {
    const tit = ['ID','품명','대여자','소속','위치','대여날짜','반납여부','반납날짜'];
            ...
            <div className="cont-head">
                <ul className="head-ul">
                    <li>
                        <label>
                            <p>전체</p>
                            <label htmlFor="total"><input type="checkbox" onChange={(e)=>allCheckHandler(e.target.checked)}/></label>
                        </label>
                    </li>
                    {tit.map((name,idx) => (
                        <li key ={idx}>
    ...
    return (
        <section className="container">
            <div className="container-tit">
                ...
                {adminBtnToggle && <AdminBtn openEvent={openAllItem} deleteEvent={deleteItem}/>}
            </div>
            {defaultToggle && <DefaultItem rentalList={list}/>}
            {allToggle && <AllItem list={allList} oneCheckHandler={onClickOneCheck} allCheckHandler={onClickAllCheck} checkedList={checkedItems} checkedAll={isAllChecked}/>}
        </section>
