  ...
  return (
    <form onSubmit={clickEvent}>
      <ul className="body-ul">
        <li></li>
        <li>
          <input name="server_name" type="textbox" onChange={changeHandler} />
        </li>
        <li>
          <input name="ip" type="textbox" onChange={changeHandler} />
        </li>
        <li>3030</li>
        <li>
          <input name="cpu_core" type="textbox" onChange={changeHandler} />
        </li>
        <li>
          <input name="disk" type="textbox" onChange={changeHandler} />
        </li>
        <li>
          <input name="memory" type="textbox" onChange={changeHandler} />
        </li>
...
const ServerContent = ({ entryId, entryName }) => {
  ...
  const currentURL = "http://localhost:3103/api/getServerData.php"; //데이터 출력시 api url
    ...
    const fetchList = async () => {
      try {
        ...
        const res = await axios.get(
          currentURL + "?parm=2&entry_id=" + currentId
        ); //equipment 불러오는 axios
        const res2 = await axios.get(
          currentURL + "?parm=4&entry_id=" + currentId
        ); //count 불러오는 axios
        setList(res.data);
        setNum(res2.data);
  ...
  useEffect(() => {
    if (sessionStorage.getItem("name") !== null) setOpenBtn(true);
  }, [session]);
  ...
  async function openDelete() {
    if (checkedItems.size > 0) {
      for (let id of checkedItems) {
        ...
        await axios
          .get(deleteURL + "?table=server&item_id=" + id)
          .then((res) => {
            // console.log(res)
          });
      }
