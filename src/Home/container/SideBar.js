...
const SideBar = ({ currentURL, clickEvent, tableName }) => {
  ...
  const insertURL = "http://localhost:3103/api/getInsertEntry.php";
  const deleteURL = "http://localhost:3103/api/getDeleteEntry.php";
  ...
  function inputEnter(event) {
    if (event.key === "Enter") {
      ...
      if (entryName.length > 0) {
        axios
          .get(insertURL + "?table=" + tableName + "&entry_name=" + entryName)
          .then((res) => {
            // console.log(res)
          });
      }
  ...
  async function onClickRemove() {
    ...
    for (let id of checkedItems) {
      ...
      const cnt = await axios.get(
        deleteURL + "?table=" + tableName + "&entry_id=" + id
      ); //선택한 entry_id와 이어진 equipment가 있는지 확인한다.
      if (cnt.data <= 0) {
        //없으면 삭제 진행
        await axios
          .get(deleteURL + "?table=" + tableName + "&item_id=" + id)
          .then((res) => {
            // console.log(res)
          });
      }
  ...
  const allClickCheck = (isChecked) => {
    if (isChecked) {
      entryList.map((item) => checkedItems.add(item.id));
      setCheckedItems(checkedItems);
      setAllChecked(true);
    } else {
      checkedItems.clear();
    ...
    <section className="sidebar">
      <div className="inner">
        ...
        {isRemove && (
          <RemoveEntry
            list={entryList}
            onCheckedSingle={oneClickCheck}
            removeEvent={onClickRemove}
            onCheckedAll={allClickCheck}
        ...
        <ul className="sidebar-ul">
          {isInsert && (
            <InsertEntry
              enterEvent={inputEnter}
              inText={inputText}
              onClickEvent={onClickInsert}
            />
          )}
        </ul>
      </div>
