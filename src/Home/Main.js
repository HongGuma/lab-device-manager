...
class Main extends React.Component {
  constructor(props) {
    ...
    this.state = {
      ...
      url: {
        0: "http://localhost:3103/api/getOfficeData.php",
        1: "http://localhost:3103/api/getServerData.php",
        2: "http://localhost:3103/api/getLabData.php",
        // 3:'http://localhost:3103/api/getSampleData.php',
      },
      currentID: 0,
      currentTit: "사무용품",
      currentURL: "",
        ...
        <section className="main-current">
          ...
          <ul className="cont-ul">
            {this.state.tabNum.map((idx) => (
              <li
                key={idx}
                onClick={() => this.onClickTab(idx, this.state.tabTitle[idx])}
              >
                <img src={this.state.tabImg[idx]} alt="main-icon" />
                <p>{this.state.tabTitle[idx]}</p>
              </li>
            ))}
          </ul>
